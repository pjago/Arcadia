         ...
         rfn (fn rfn [bvec [t k v]]
               (case t
                 ...
                 :props (reduce (fn [bvec prop]
                          ...
                          bvec
                          v)
                 :state (reduce (fn [bvec prop]
                                  (conj bvec (symbol (name prop)) 
                                        `(arcadia.core/state ~gsym ~(keyword (re-find #"[^:].*" (str prop))))))
                          bvec
                          v)
                 :cmpt (reduce (fn [bvec [lhs rhs]]
                                 (clet [form (-> `(arcadia.core/cmpt ~gsym ~rhs) (tagged rhs))]
...
(defmethod destructuring-form 'arcadia.sugar/cmpt [bvec b v]
